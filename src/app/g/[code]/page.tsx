...
export default function GiftJoinPage({ params }: PageProps) {
  ...
  useEffect(() => {
    // Fetch gift and tribe data
    fetch(`/api/gift/${resolvedParams.code}`)
      .then(res => res.json())
      .then(data => {
        if (data.error) {
          setError(data.error);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email.trim() || isSubmitting || !giftData) return;
    ...
    try {
      ...
      const response = await fetch("/api/join", {
        ...
        body: JSON.stringify({
          slug: giftData.tribe.slug,
          email: email.trim(),
          baseUrl,
          giftCode: resolvedParams.code,
        }),
      });
        ...
        <div className="flex flex-col items-center text-center">
          ...
          >
            ...
            <div className="flex justify-center mb-3">
              {giftData?.gift.thumbnailUrl ? (
                <Image
                  src={giftData.gift.thumbnailUrl}
                  alt={giftData.gift.fileName}
                  width={64}
                  height={64}
